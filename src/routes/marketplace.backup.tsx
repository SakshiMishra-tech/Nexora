...
export const Route = createFileRoute("/marketplace/backup")({
  ...
  return (
    <ModuleAccessBoundary moduleId="marketplace">
      <main className="min-h-screen bg-background text-foreground">
        ...
        {/* SiteNav removed — shown only on home dashboard */}
          ...
          {activeView === "browse" && (
            <div className="mb-6">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="rounded-full border border-border bg-card px-3 py-1 text-[10px] font-black text-muted-foreground uppercase tracking-wider">
                  Sample Listings
                </span>
              </div>
              <div className="grid gap-4 sm:grid-cols-3">
                {DEMO_LISTINGS.map((item) => (
                  <div
                    key={item.id}
                    className="group relative overflow-hidden rounded-2xl border border-border bg-card shadow-soft transition-all duration-200 hover:-translate-y-1 hover:shadow-mega"
                  >
                    <div className="relative h-48 overflow-hidden bg-secondary">
                      ...
                      <span className={`absolute left-3 top-3 rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${item.condition === "LIKE NEW" ? "bg-emerald-500 text-white" : "bg-amber-500 text-white"}`}>
                        {item.condition}
                      </span>
                    </div>
                    <div className="p-4 space-y-2">
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <span className="text-base font-black text-foreground">₹{item.price.toLocaleString("en-IN")}</span>
                        <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-bold text-primary">{item.category}</span>
                      </div>
                      <div className="flex items-center gap-1.5 border-t border-border/60 pt-2">
                        <div className="grid h-5 w-5 place-items-center rounded-full bg-primary text-[9px] font-black text-primary-foreground shrink-0">{item.seller[0]}</div>
                        <p className="text-[11px] text-muted-foreground font-semibold truncate">{item.seller} · {item.campus}</p>
                      </div>
                    </div>
                  </div>
                ))}
